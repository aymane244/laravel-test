...
<div id="myModal" class="modal_confirm modal">
    ...
    <div class="modal-content">
        ...
        <p>Voulez-vous vraiment supprimer cet membre dans cette groupe? Ce processus ne peut pas être annulé.</p>
        </div>

        <div class="modal-footer">
            <button class="btn btn-info" onclick="$('.window').css('display','none');$('.modal').css('display','none');">Cancel</button>
            <button class="btn btn-danger" onclick="event.preventDefault();document.getElementById('delete_membre_groupe').submit();" id="submit_confirmation">Supprimer</button>
        </div>

        <form id="delete_membre_groupe" action="/remove_user/{{$group_id}}" method="POST" class="d-none">
            @csrf
...
<div class="mt-5" style="text-align: center;margin-bottom: 90px">
    <h1 style="font-weight: 900">Les membres de {{ $group->name }}</h1>
...
<div class="container">
             ...
             <div class="card m-b-30">
                 ...
                 <div class="card-body">
                     ...
                     <div class="btn-group btn-group-sm div_entreprise_action" style="">
                         <a href="#" onclick="confirmer('{{$user->id}}')" class="btn btn-primary tooltips btn_action"><i class="fa fa-trash" aria-hidden="true"></i>&nbsp Supprimer</a>
                     </div>
    ...
    <div style="margin: 0 auto;width:20%">{{ $group_members->links() }}</div>
 </div>
@endsection
@section('script')
